...
class GetNearbyPlacesCubit extends Cubit<GetNearbyPlacesStates> {
  ...
  Future getNearbyPlaces({
    ...
    try {
      ...
      List<NearbyPlacesModel> nearbyPlaces = await GetNearbyPlacesService()
          .getNearbyPlaces(lat: lat, lng: lng, type: type);
  ...
  Future getAllNearbyPlaces({required double lat, required double lng}) async {
    try {
      ...
      for (final type in types) {
        final places = await GetNearbyPlacesService().getNearbyPlaces(
          lat: lat,
          lng: lng,
          type: type,
        );
  ...
  Future getCarServiceNearbyPlaces({
    required double lat,
    required double lng,
  }) async {
    try {
